...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        /** "TODO": Your code here. */
        while (inBounds(parentIndex(index)) && contents[index].myPriority
    ...
    private void sink(int index) {
        ...
        while (inBounds(rightIndex(index)) && contents[index].myPriority
            ...
            int minNodeIndex = min(leftIndex(index), rightIndex(index));
            swap(index, minNodeIndex);
            index = minNodeIndex;
        }
        while (inBounds(leftIndex(index)) && contents[index].myPriority
                > contents[leftIndex(index)].myPriority) {
    ...
    public void insert(T item, double priority) {
        ...
        /* "TODO": Your code here! */
        Node addItem = new Node(item, priority);
        contents[++size] = addItem;
        swim(size);
    }
    ...
    public T removeMin() {
        ...
        if (size >= 1) {
            swap(1, size);
            removeItem = contents[size--].myItem;
            sink(1);
        }
        return removeItem;
    }
